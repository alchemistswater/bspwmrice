...
sudo pacman -S  go ttf-joypixels ttf-croscore noto-fonts-cjk noto-fonts \
	nextcloud-client gvim scrot htop arc-gtk-theme firefox sxhkd \
	zathura-pdf-mupdf xclip gnome-keyring xfce4-notifyd xsel xdotool \
	xorg-server xorg-xinit xorg-xrdb xorg-xprop xdo pulseaudio-alsa \
	exa pavucontrol tmux bash-completion pamixer pulseaudio fzf cmus \
	bspwm feh git openssh slock ttf-roboto ttf-roboto-mono dash
...
yay -S  picom pfetch-git moka-icon-theme lxappearance sox imagemagick \
	i3lock ttf-font-awesome lemonbar-xft-git st-luke-git
...
case $yna in
	...
	lib32-alsa-plugins lutris lutris-wine-meta itch
